...
def regular_department(db, regular_user):
    return Department.objects.create(name='Test Department', head_user_id=regular_user.id)
...
class TestDepartmentViewSet:
    ...
    def test_put_department_as_selfManager(self,api_client,regular_department,regular_user):
        api_client.force_authenticate(user=regular_user)
        response = api_client.put(f'/department/{regular_department.id}/',{},format='json')
        assert response.status_code != status.HTTP_403_FORBIDDEN


    def test_patch_department_as_selfManager(self,api_client,regular_department,regular_user):
        api_client.force_authenticate(user=regular_user)
        response = api_client.put(f'/department/{regular_department.id}/', {}, format='json')
        assert response.status_code != status.HTTP_403_FORBIDDEN
...
class TestDepartMemberViewSet:
    ...
    def test_delete_depart_member_as_selfManager(self):
        pass
...
class TestDepartmentRequestViewSet:
    ...
    def test_approve_department_request_as_selfManager(self):
        pass

    def test_reject_department_request_as_selfManager(self):
        pass
